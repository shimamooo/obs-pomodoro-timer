...
function set_time_text()
	...
	if cur_seconds < 1 then
		text = stop_text
		-- play jingle
		playsound(script_path() .. "bell.wav")
	end
	...
	if text ~= last_text then
		...
		if source ~= nil then
			...
			obs.obs_data_set_string(settings, "text", "Session #" .. session .. "\n" .. text)
...
function script_properties()
	...
	obs.obs_properties_add_text(props, "session", "Session Number", obs.OBS_TEXT_DEFAULT)
...
function script_update(settings)
	...
	session = obs.obs_data_get_string(settings, "session")
...
function script_defaults(settings)
	obs.obs_data_set_default_int(settings, "duration", 75)
